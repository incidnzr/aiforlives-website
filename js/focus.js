...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (focusNavItems.length > 0 && focusSections.length > 0) {
        ...
        function handleHashChange() {
            ...
            if (hash && (hash === 'media' || hash === 'health' || hash === 'environment')) {
                const targetNav = document.querySelector(`.focus-nav-item[data-target="${hash}"]`);
                if (targetNav) {
                    targetNav.click();
                }
            }
    ...
    const focusCards = document.querySelectorAll('.project-card, .highlight-card, .env-topic');
    ...
    function updatePageTitle() {
        ...
        if (activeSection) {
            const sectionName = activeSection.id.charAt(0).toUpperCase() + activeSection.id.slice(1);
